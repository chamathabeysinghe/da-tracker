...
class DETRTrackingBase(nn.Module):
    ...
    def __init__(self,
                 target_domain_track_query_threshold=0.85,
        ...
        self.target_domain_track_query_threshold = target_domain_track_query_threshold
    ...
    def forward(self, samples: NestedTensor, targets: list = None, target_domain=False, postprocessors=None):
        if target_domain:
            prev_out, _, _, _, _, prev_d_pixel, prev_domain_p, prev_domain_decoder = super().forward([targets[0]['prev_image']], target_domain=True)
            results = postprocessors['bbox'](prev_out, targets[0]['orig_size'].reshape(1, 2))
            num_object_queries = 100
            detection_obj_score_thresh = self.target_domain_track_query_threshold
            keep_indices = []
            for i in range(len(results)):
                result = results[i]
                new_det_scores = result['scores'][-num_object_queries:]
                new_det_keep = torch.logical_and(
                    new_det_scores > detection_obj_score_thresh,
                    result['labels'][-num_object_queries:] == 0)
                new_det_keep = (new_det_keep == True).nonzero(as_tuple=True)[0]
                keep_indices.append(new_det_keep)
            for i, (target, prev_ind) in enumerate(zip(targets, keep_indices)):
                prev_out_ind = prev_ind
                prev_out_ind = torch.tensor(prev_out_ind.tolist()).long()
            ...
            _, _, _, _, _, d_pixel, domain_p, domain_decoder = super().forward(samples, targets, target_domain=True)
            return d_pixel, domain_p, domain_decoder, prev_d_pixel, prev_domain_p, prev_domain_decoder
        prev_domain_p = None
        prev_d_pixel = None
        prev_domain_p_decoder = None
        if targets is not None and not self._tracking:
            prev_out, _, _, _, _, prev_d_pixel, prev_domain_p, prev_domain_p_decoder = super().forward([targets[0]['prev_image']])
        ...
        out, targets, features, memory, hs, d_pixel, domain_p, domain_p_decoder = super().forward(samples, targets)

        return out, targets, features, memory, hs, d_pixel, domain_p, domain_p_decoder, prev_d_pixel, prev_domain_p, prev_domain_p_decoder
