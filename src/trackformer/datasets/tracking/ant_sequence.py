...
class AntSequence(Dataset):
    ...
    def __init__(self, root_dir: str = 'data', seq_name: Optional[str] = None,
                 vis_threshold: float = 0.0, img_transform: Namespace = None, data_folder: str = "tracking_target_full") -> None:
        """
        Args:
        ...
        self._data_dir = osp.join(root_dir, data_folder)

        self._train_folders = []
        self._val_folders = []
        self._test_folders = []

        if os.path.exists(os.path.join(self._data_dir, 'train_split')):
            self._train_folders = os.listdir(os.path.join(self._data_dir, 'train_split'))
        if os.path.exists(os.path.join(self._data_dir, 'val_split')):
            self._val_folders = os.listdir(os.path.join(self._data_dir, 'val_split'))
        if os.path.exists(os.path.join(self._data_dir, 'test_split')):
            self._test_folders = os.listdir(os.path.join(self._data_dir, 'test_split'))
        ...
        if seq_name is not None:
            ...
            assert full_seq_name in self._train_folders or full_seq_name in self._val_folders or full_seq_name in self._test_folders, \
    ...
    def get_seq_path(self) -> str:
        ...
        if full_seq_name in self._train_folders:
            return osp.join(self._data_dir, 'train_split', full_seq_name)
        elif full_seq_name in self._test_folders:
            return osp.join(self._data_dir, 'test_split', full_seq_name)
        else:
            return osp.join(self._data_dir, 'val_split', full_seq_name)
