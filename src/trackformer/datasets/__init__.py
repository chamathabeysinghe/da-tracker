...
from .mot import build_mot, build_mot_crowdhuman, build_mot_target_domain
...
def build_dataset_target_domain(split: str, args: Namespace) -> Dataset:
    """Helper function to build dataset for different splits ('train' or 'val')."""
    if args.dataset == 'mot':
        dataset = build_mot_target_domain(split, args)
    else:
        raise ValueError(f'dataset {args.dataset} not supported')
