...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, postprocessors,
                    ...
                    device: torch.device, epoch: int, FL1, FL2, visualizers: dict, args):
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, epoch)):
        ...
        outputs, targets, _, _, _, out_d_pixel, out_d, out_d_decoder, prev_out_d_pixel, prev_out_d, prev_out_d_decoder\
            = model(samples, targets, postprocessors=postprocessors)
        ...
        dloss_s = 0.5 * FL1(out_d, domain_s)
        dloss_s_p = torch.mean(out_d_pixel ** 2) * 0.5

        prev_dloss_s = 0.5 * FL1(prev_out_d, domain_s)
        prev_dloss_s_p = torch.mean(prev_out_d_pixel ** 2) * 0.5

        # global decoder alignment loss
        domain_decoder_s = torch.zeros(out_d_decoder.size(0)).long().cuda()
        dloss_decoder_s = 0.5 * FL2(out_d_decoder, domain_decoder_s)
        domain_decoder_s = torch.zeros(prev_out_d_decoder.size(0)).long().cuda()
        prev_dloss_decoder_s = 0.5 * FL2(prev_out_d_decoder, domain_decoder_s)
        ...
        out_d_pixel, out_d, out_d_decoder, prev_out_d_pixel, prev_out_d, prev_out_d_decoder = model(samples_t, targets_t, target_domain=True, postprocessors=postprocessors)
        domain_t = torch.ones(out_d.size(0)).long().cuda()
        domain_decoder_t = torch.ones(out_d_decoder.size(0)).long().cuda()
        dloss_t = 0.5 * FL1(out_d, domain_t)
        dloss_t_p = torch.mean((1 - out_d_pixel) ** 2) * 0.5
        dloss_decoder_t = 0.5 * FL2(out_d_decoder, domain_decoder_t)

        prev_domain_decoder_t = torch.ones(prev_out_d_decoder.size(0)).long().cuda()
        prev_dloss_t = 0.5 * FL1(prev_out_d, domain_t)
        prev_dloss_t_p = torch.mean((1 - prev_out_d_pixel) ** 2) * 0.5
        prev_dloss_decoder_t = 0.5 * FL2(prev_out_d_decoder, prev_domain_decoder_t)

        # total discriminator loss
        dloss_total = (dloss_s_p + prev_dloss_s + dloss_t_p + prev_dloss_t_p) * args.disc_loss_coef_local \
                      + (dloss_s + prev_dloss_s_p + dloss_t + prev_dloss_t) * args.disc_loss_coef_global \
                      + (dloss_decoder_s + prev_dloss_decoder_s + dloss_decoder_t + prev_dloss_decoder_t) * args.disc_loss_decoder_coef_global
        losses += dloss_total
        ...
        loss_dict_reduced_unscaled['global_discriminator_decoder_loss_source'] = dloss_decoder_s
        ...
        loss_dict_reduced_unscaled['global_discriminator_decoder_loss_target'] = dloss_decoder_t

        loss_dict_reduced_unscaled['prev_discriminator_loss_source'] = prev_dloss_s_p
        loss_dict_reduced_unscaled['prev_global_discriminator_loss_source'] = prev_dloss_s
        loss_dict_reduced_unscaled['prev_global_discriminator_decoder_loss_source'] = prev_dloss_decoder_s
        loss_dict_reduced_unscaled['prev_discriminator_loss_target'] = prev_dloss_t_p
        loss_dict_reduced_unscaled['prev_global_discriminator_loss_target'] = prev_dloss_t
        loss_dict_reduced_unscaled['prev_global_discriminator_decoder_loss_target'] = prev_dloss_decoder_t

        loss_dict_reduced_scaled = {
            k: v * weight_dict[k] for k, v in loss_dict_reduced.items() if k in weight_dict}
        trackformer_loss_value = sum(loss_dict_reduced_scaled.values()).item()
        ...
        metric_logger.update(loss=loss_value, trackformer_loss=trackformer_loss_value,
                             **loss_dict_reduced_scaled,
                             **loss_dict_reduced_unscaled)
...
def evaluate(model, criterion, postprocessors, data_loader, device,
    ...
    if args.tracking and args.tracking_eval:
        ...
        run = ex.run(config_updates={
            ...
            'data_folder': args.seq_data_folder,
